...
static void SetupMountNamespace() {
  ...
  if (mount(nullptr, "/", nullptr, MS_REC | MS_PRIVATE, nullptr) < 0) {
    std::cerr << "Is this running in a container that's doing something "
                 "weird with device cgroups?" << std::endl;
    DIE("mount");
  }
}
...
static void MountFilesystems() {
  ...
  if (mount("/tmp", "/tmp", nullptr, MS_BIND,
            nullptr) < 0) {
    DIE("mount(%s, %s, nullptr, MS_BIND, nullptr)", "/tmp", "/tmp");
  }

  if (mount("/proc", "/proc", nullptr, MS_BIND,
            nullptr) < 0) {
    DIE("mount(%s, %s, nullptr, MS_BIND, nullptr)", "/proc", "/proc");
  }

  if (mount("/dev", "/dev", nullptr, MS_BIND,
            nullptr) < 0) {
    DIE("mount(%s, %s, nullptr, MS_BIND, nullptr)", "/dev", "/dev");
  }
...
static void DropPrivs(const std::string& unpriv_user) {
  passwd* user = getpwnam(unpriv_user.c_str());
  if (!user) {
    std::cerr << "error: " << strerror(errno) << std::endl;
    DIE("DropPrivs() user");
  }
  auto* grp = getgrnam(unpriv_user.c_str());
  if (!grp) {
    std::cerr << "error: " << strerror(errno) << std::endl;
    DIE("DropPrivs() group");
  }
  ...
  if (setresgid(target_gid, target_gid, target_gid) != 0) {
    std::cerr << "error: " << strerror(errno) << std::endl;
    DIE("DropPrivs() setresgid");
  }
  if (setresuid(target_uid, target_uid, target_uid) != 0) {
    std::cerr << "error: " << strerror(errno) << std::endl;
    DIE("DropPrivs() setresuid");
  }
}

void Atexit() {
  //system("ps -ax -o pid,ppid,command | grep  'sleep'");
  kill(-getpid(), 9);
  std::cerr << "Kill failed!" << std::endl;
  DIE("Confusion ensues.");
}
...
static void SpawnChild() {
  ...
  if (global_child_pid < 0) {
    ...
    if (opt.switch_to_user.size()) {
      DropPrivs(opt.switch_to_user);
    }
