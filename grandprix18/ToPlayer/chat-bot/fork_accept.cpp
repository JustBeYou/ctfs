...
extern int chatbot(int fd, struct sockaddr* clientaddress, char** argv);
...
int main(int argc, char **argv) {
	...
	if (argc != 5) {
		printf("Usage: %s <record-file> <secret-key-file> <super-secret-key-file> <secret-data>\n", argv[0]);
		exit(1);
	}
    ...
    while (1) {
        clientfd = accept(sockfd, &clientaddress, (socklen_t *)&address_len);
        if (clientfd == -1) {
            perror("accept");
            continue;
        }
        ...
        if (pid == 0) {
            ...
            rc = drop_privs("a51");
            if (rc == 0) {
                rc = chatbot(clientfd, &clientaddress, argv);
            }
