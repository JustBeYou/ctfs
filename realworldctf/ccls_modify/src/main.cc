...
void Hack() { 
    printf("JSON: \n");
    std::string path = "fringe.cc.blob";
  std::optional<std::string> serialized = ReadContent(path);
  std::unique_ptr<IndexFile> result = ccls::Deserialize(SerializeFormat::Binary, "fringe.cc", *serialized, "int b;", std::nullopt /*expected_version*/);
...
int main(int argc, char** argv) {
  ...
  if (opt_log_file.size() || opt_log_file_append.size()) {
    ...
    //ccls::pipeline::RawCacheLoadHack();
